         ...
         <div class="carousel-item">
            <div class="row slide mx-auto">
               ...
               <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6">
                  ...
                  <p><b>Nota:</b> ${filmeCards.results[i].vote_average}</p>
                  <button type="button" id="${filmeCarrousel.results[i].id}" class="btn btn-dark" onclick="abrirPag()">Detalhes</button>
               </div>
            </div>
         </div>`);
      }
...
function exibeCardsPesquisa(evento) {
   window.location.href = "pesquisa.html"
   $('#cards-index').html('');
   let filmeCardsPesquisa = JSON.parse(evento.target.responseText);
   for(let i = 0; i < filmeCardsPesquisa.results.length; i++) {
      let data = filmeCards.results[i].release_date;
      data = data.split('-').reverse('').join('/');
      $('#cards-index').append(`
      <div class="col-12 col-sm-12 col-md-6 col-lg-4 col-xl-3">  
